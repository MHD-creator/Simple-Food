...
export const createLivreur = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { name, telephone: telInput, password } = req.body as { name: string; telephone: string; password: string };
...
export const getLivreurs = async (req: Request, res: Response): Promise<void> => {
  try {
    const cuisinierId = (req as AuthRequest).user.userId;
...
export const updateLivreur = async (req: Request, res: Response): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({ success: false, message: 'Erreurs de validation', errors: errors.array() });
    ...
    const cuisinierId = (req as AuthRequest).user.userId;
    const { id } = req.params;
    const { name, telephone: telInput, password } = req.body as {
      name?: string;
      telephone?: string;
      password?: string;
    };
    ...
    if (name !== undefined) livreur.name = name;
    if (telInput !== undefined) {
      const telephone = normalizePhone(telInput);
      if (telephone !== livreur.telephone) {
        const exists = await User.findOne({ telephone });
        if (exists && String((exists as any)._id) !== String((livreur as any)._id)) {
          res.status(400).json({ success: false, message: 'Ce numéro de téléphone est déjà utilisé' });
          return;
        }
...
export const deleteLivreur = async (req: Request, res: Response): Promise<void> => {
  try {
    const cuisinierId = (req as AuthRequest).user.userId;
    const { id } = req.params;
